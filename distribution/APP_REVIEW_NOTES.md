trackme records outdoor walks and runs using Core Location. No account or login is required.
...
HealthKit is used only to write user-authorized completed walking or running workouts, walking/running distance, and workout routes. If a synced workout is deleted from trackme, the matching Apple Health workout, distance sample, and route are deleted too. The app does not read Health data and does not use Health data for advertising or data mining.

Background Location:

Background location updates are enabled only while a workout is actively recording. Pausing or finishing a workout stops location updates. The location indicator is shown while background tracking is active.
